...
def editING():
    ...
    #     con.row_factory = sqlite3.Row
    #     cur = con.cursor()
    #     q = "select * from ingredienser where idIngredienser "
    #   #  cur.execute("""select * from ingredienser where idIngredienser """)
    #     cur.execute(q,[qid__input])
    #     rows = cur.fetchall()
    #     cur.close()
    #     con.close()
...
def lavis():
    ...
    if request.method == "POST":
        ...
        #cur.execute("update ingredienser set inventar = inventar - mængde from forbindelse where ingredienser.idIngredienser = forbindelse.fkis")
        ...
        return render_template("vis_ingredienser.html")
        
    else:
        #vis alle is sammensætning database
        con = sqlite3.connect("/Users/win/Desktop/is/butikken.db")
        con.row_factory = sqlite3.Row
        cur = con.cursor()
#        sqll = """SELECT * from forbindelse WHERE fkis = ()  """

        sqll = """SELECT * from ice """
      #  cur.execute(sqll,skrive_input)
        cur.execute(sqll)
